...
ARQUIVO_ESTOQUE = "estoque_atual.xlsx"
...
if "categorias" not in st.session_state:
    st.session_state.categorias = {cat: [] for cat in CATEGORIAS_FIXAS}
    with open(ARQUIVO_CATEGORIAS, "w") as f:
        json.dump(st.session_state.categorias, f, indent=2)

# Carregar estoque
# Carregar estoque direto do código (modo fixo)
...
# ---------- RETIRADA ----------
    ...
    for item in itens_categoria:
        ...
        with col1:
            qtd = st.number_input(f"Quantidade a retirar ({item['nome']})", 0, item["quantidade"], step=1, key=f"qtd_{item['nome']}")
        with col2:
            if st.button(f"Retirar - {item['nome']}", key=f"btn_{item['nome']}"):
                if "tecnico_selecionado" not in st.session_state:
                    st.error("Selecione um técnico primeiro.")
                    ...
                    salvar_estoque()
                    st.success(f"{qtd} retirado(s) de {item['nome']}.")
